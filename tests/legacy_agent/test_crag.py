...
from legacy.agent.graph import AgentConfig, build_ReAct_agent, route_after_crag
from legacy.agent.reflection.parsers import extract_rag_tool_results, split_rag_chunks
from legacy.agent.state import DEFAULT_RAG_TOOL_NAME, AgentState
from legacy.agent.subgraph.CRAG import (
    CragConfig,
    build_crag_node,
    build_crag_subgraph,
    compute_verdict,
...
from legacy.agent.subgraph.score_fn import build_cross_encoder_score_fn
...
def test_build_agent_react_crag_pattern():
    ...
    from legacy.agent.graph import build_agent
