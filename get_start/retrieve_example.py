...
_INDEX_PROFILE_ID = "baseline"
_RETRIEVE_PROFILE_ID = "rerank_contextual"
_COLLECTION = f"getstart_codex_{_INDEX_PROFILE_ID}"
...
async def main() -> None:
    ...
    # 2. 读取 arg_config.yaml：index / retrieve profile 开关 + retriever.top_k。
    #    _COLLECTION 由 index profile 决定，须与 index_example 一致且已 index 过。
    config = get_rag_config()
    retrieve_profile = get_retrieve_profile(config, _RETRIEVE_PROFILE_ID)
    top_k = config.retriever.top_k

    # 3. 按 retrieve profile 组装 RAGRetriever：query transform → retrieve → rerank。
    ...
    write_retrieve_traces_json(
        ...
        meta=RetrieveRunMeta(
            profile_id=f"{_INDEX_PROFILE_ID}+{_RETRIEVE_PROFILE_ID}",
